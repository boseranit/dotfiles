...
- `system/<distribution>.txt` contains the small OS-level foundation.
...
Applications used only by one host belong in that host's `machines/` document,
not in the shared system package list.
...
5. Record machine-only applications in `machines/`; do not add them globally.
